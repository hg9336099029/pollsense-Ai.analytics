...
const VotedPolls = () => {
  ...
  useEffect(() => {
    const fetchPolls = async () => {
      try {
        setLoading(true);
        const response = await axiosInstance.get(API_PATH.AUTH.GET_VOTED_POLLS);
        const fetchedPolls = response.data.votedPolls || [];
  ...
  const handleVote = async (pollId, optionIndex) => {
    try {
      ...
      toast.error(error.response?.data?.message || 'You have already voted on this poll');
    }
  };
      ...
      <DashboardLayout>
        <div className="flex items-center justify-center h-[calc(100vh-64px)]">
          <div className="text-center">
            ...
            <p className="text-gray-600">Loading voted polls...</p>
          </div>
        </div>
      </DashboardLayout>
    );
    ...
    <DashboardLayout>
      <div className="flex-1 overflow-auto bg-gradient-to-br from-gray-50 to-gray-100 p-6">
        <div className="max-w-6xl mx-auto">
          {/* Header */}
          <div className="mb-8">
            <h1 className="text-4xl font-bold text-gray-900">Voted Polls</h1>
            <p className="text-gray-600 mt-2">Polls you've already voted on • {polls.length} total</p>
          </div>
          ...
          {filteredPolls.length === 0 ? (
            <div className="text-center py-16 bg-white rounded-xl border border-gray-200">
              ...
              <p className="text-gray-600 text-lg">No voted polls found</p>
              <p className="text-gray-500">Explore and vote on polls to see them here</p>
            </div>
          ) : (
                        ...
                        <>
                          {poll.options.map((option, idx) => (
                            ...
                            >
                              ...
                              <div className="bg-gray-200 h-2 rounded-full mt-2" style={{ width: '100%' }}>
